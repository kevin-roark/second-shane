...
THREE.PointerLockControls = function ( camera ) {
	...
	this.update = function () {
		...
		// velocity determination based on moving count stamina
		if (moveForward || moveBackward || moveLeft || moveRight) {
			movingCount = Math.min(440, movingCount + 1.5);
		} else {
			movingCount = Math.max(0, movingCount - 9);
		}

		var velocityBoost = 800 - movingCount;

		if (moveForward) velocity.z += velocityBoost * delta;
		if (moveBackward) velocity.z -= velocityBoost * delta;

		if (moveLeft) velocity.x += velocityBoost * delta;
		if (moveRight) velocity.x -= velocityBoost * delta;
